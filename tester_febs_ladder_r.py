    ...
    for se in setup_elements:
        ...
        for uplink in se.uplinks:
            if se.downlink == 0:
                ...
                if (uplink < 16 or uplink > 23):
                    remove_list_01.append(uplink)
                    log.info("Uplink appended to remove list: {}".format(uplink))
            elif se.downlink == 1:
                if (uplink < 8 or uplink >15):
                    remove_list_23.append(uplink)
                    log.info("Uplink appended to remove list: {}".format(uplink))
            elif se.downlink == 3:
                if (uplink < 24 or uplink >31):
                    remove_list_23.append(uplink)
                    log.info("Uplink appended to remove list: {}".format(uplink))
            elif se.downlink == 4:
                if (uplink < 32):
                ...
                pass
        if se.downlink == 0 or se.downlink == 2 or se.downlink == 4:
...
def reading_lv(pol_usr = 'N'):
    ...
    if (pol == 'N' or pol == '0'):
        ...
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u204 voltage | grep -Eo '[0-9]+([.][0-9]+)' > lv_iv.txt")
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u205 voltage | grep -Eo '[0-9]+([.][0-9]+)' >> lv_iv.txt")
        #os.system("./../../../lv/mpod_hhlab_2024/lab24_mpod02_u204 voltage | grep -Eo '[0-9]+([.][0-9]+)' > lv_iv.txt")
        #os.system("./../../../lv/mpod_hhlab_2024/lab24_mpod02_u205 voltage | grep -Eo '[0-9]+([.][0-9]+)' >> lv_iv.txt")
    elif(pol == 'P' or pol == '1'):
        print("Reading LV for P-side (holes polarity):")
        #Reading Voltage and current values and writting them in a file                                                               
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u206 voltage | grep -Eo '[0-9]+([.][0-9]+)' > lv_iv.txt")
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u207 voltage | grep -Eo '[0-9]+([.][0-9]+)' >> lv_iv.txt")
        #os.system("./../../../lv/mpod_hhlab_2024/lab24_mpod02_u206 voltage | grep -Eo '[0-9]+([.][0-9]+)' > lv_iv.txt")
        #os.system("./../../../lv/mpod_hhlab_2024/lab24_mpod02_u207 voltage | grep -Eo '[0-9]+([.][0-9]+)' >> lv_iv.txt")
    else:
...
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
emu_list = ["EMU_236"]                          #["EMU_213"]     # List of EMU boards used during the test
# Ladder name or module path
module_path  = "ladder_files/"                   					  # Path to store all the information concerning sll modules of the ladder under test
ladder_sn = "FEB_TEST"
module_sn = " "                                						  		  # Initiialization of the module SN
calibration_data_path = "/home/cbm/cbmsoft/emu_test_module_arr/python/module_files"
...
module_dir = module_path + "/" + str(ladder_sn) + "/" + str(module_sn)
...
#trim_dir = calibration_data_path + "/" + str(ladder_sn) + "/" + str(module_sn) + "/trim_files/"
print(trim_dir)
...
'''
for emu in emu_list:
	for downlinks in active_downlinks_map[emu]:
		active_downlinks.append(downlinks)
...
for test_step in test_list:
    if (test_step == "power_on_emu"):
        ...
        smx_l_nside = smx_l[p_asics:p_asics + n_asics]
        smx_l_pside = smx_l[0:p_asics]
        #smx_l_pside = smx_l[8:16]
        #smx_l_nside = smx_l[0:8]
